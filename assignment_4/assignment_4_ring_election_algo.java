/*
    ...
    Election - Ring : Implement a Ring Election Algorithm for distributed systems in Java. 
    In this algorithm assume that the link between the process are unidirectional and 
    every process can message to the process on its right only. 
    Each process is associated with name, priority and state (Active/Inactive). 
    Accept the number of processes from the user with their priorities and states. 
    Select the process with highest priority as coordinator. 
    Allow user to choose any active process to initialize Election. 
*/
